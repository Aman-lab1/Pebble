# 🪨 Pebble

A minimalist, offline-first expense tracker built for people who want to record expenses in seconds—not minutes.

---

## 🚀 Live Demo

**Try Pebble here**

https://aman-lab1.github.io/Pebble/

> For the best experience, install Pebble as a Progressive Web App (PWA).

Pebble is built around one simple philosophy:

> Spend less time tracking your money, and more time living your life.

No accounts.  
No ads.  
No subscriptions.  
Your data stays on your device.

---

## ⭐ Highlights

- 🔒 Privacy-first — your data never leaves your device
- 📶 Works completely offline
- 📱 Installable as a Progressive Web App (PWA)
- 📊 Built-in analytics and smart spending insights
- ⚡ Fast, minimalist, distraction-free experience

---

# ✨ Features

### 💸 Expense Tracking

- Quick expense logging
- Edit & delete expenses
- Expense Detail Sheet
- Search expenses by category, note, payment method, or amount
- Dynamic categories
- Cash & Digital payment tracking
- Remembers your last payment method

### 📊 Dashboard

- Monthly budget tracking
- Budget remaining indicator
- Spending distribution
- Category rankings
- Monthly spending charts

### 📈 Analytics

- Monthly Summary Cards
- Daily Spending Trend
- Category Breakdown
- Payment Method Breakdown
- Monthly Statistics
- Smart Spending Insights

### 🔍 Filters & Search

- Today
- Week
- Month
- Year
- All Time
- Custom Date Range
- Instant expense search

### 📤 Utilities

- CSV Export
- Native success confirmations
- Offline-first architecture
- Progressive Web App
- Local-first storage

---

# 📱 Progressive Web App

Pebble can be installed directly from your browser.

Once installed it behaves like a native mobile application:

- Home Screen installation
- Full-screen experience
- Portrait layout
- Offline support
- Fast startup
- Automatic updates

---

# 🛠 Tech Stack

- HTML5
- CSS3
- Vanilla JavaScript (ES6+)
- Chart.js
- LocalStorage
- Service Worker
- Web App Manifest
- Google Analytics 4
- Microsoft Clarity

---

# 🎨 Design Philosophy

Pebble follows a minimalist, iOS-inspired design language focused on speed, clarity, and distraction-free interactions.

Every feature is designed around one question:

> Can this help users record an expense in fewer taps?

If the answer is no, it doesn't belong in Pebble.

---

# 📲 Install Pebble

Pebble is a Progressive Web App (PWA), so there's nothing to download from an app store.

### 🍎 iPhone / iPad (Safari)

1. Open Pebble in **Safari**:

   https://aman-lab1.github.io/Pebble/

2. Tap the **Share** button (⬆️).

3. Scroll down and tap **Add to Home Screen**.

4. Tap **Add**.

Pebble will now appear on your Home Screen and launch like a native app.

---

### 🤖 Android (Chrome)

1. Open Pebble in **Google Chrome**:

   https://aman-lab1.github.io/Pebble/

2. Tap the **⋮** (three-dot menu).

3. Tap **Install**.

Pebble will now launch as a standalone app from your Home Screen.

---

# 📦 Current Version

## Pebble v1.4.0

### What's New

### 🔍 Search Experience

- Instant expense search
- Search by category
- Search by note
- Search by payment method
- Search by amount
- Improved empty-state messaging for search results

### 📄 Expense Detail Sheet

- Native bottom-sheet experience
- Complete expense information
- Quick Edit and Delete actions
- Frosted-glass backdrop
- Smooth slide-up animations

### 📊 Analytics

- Monthly Summary Cards
- Daily Spending Trend
- Category Breakdown
- Payment Method Breakdown
- Monthly Statistics
- Smart Spending Insights

### 💵 Payment Improvements

- Cash & Digital tracking
- Payment usage analytics
- Last payment method remembered

### 🛠 Developer Improvements

- Google Analytics 4 integration
- Microsoft Clarity integration
- Google Search Console verification
- Sitemap.xml & robots.txt
- Updated PWA icons and manifest
- Improved offline caching
- Cleaner analytics architecture

---

# 🗺 Roadmap

### Upcoming

- Manage Categories
- Custom Categories
- Budget alerts
- Smart notifications
- Cloud backup (optional)
- Multiple wallets
- Recurring expenses
- CSV Import
- Themes
- Multi-currency support

---

# 🤝 Contributing

Suggestions, bug reports, and feature ideas are always welcome.

---

# 📄 License

MIT License

---

# ❤️ Made with Love

Designed and developed by **Aman**.

If you enjoyed Pebble, consider giving the repository a ⭐.